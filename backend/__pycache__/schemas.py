...
class UserRead(UserBase):
    user_id: int
    created_at: datetime
    user_group_id: Optional[int] = None # UserRead에도 추가해야 함
...
class CreditsLedgerBase(BaseModel):
    type: LedgerType # type을 Enum으로 변경
    points: int
    reason: str
...
class ChallengeBase(BaseModel):
    title: str
    description: Optional[str] = None
    scope: ChallengeScope = ChallengeScope.PERSONAL # Enum으로 변경
    target_mode: ChallengeTarget = ChallengeTarget.ANY # Enum으로 변경
    target_saved_g: int
    start_at: datetime
    end_at: datetime
...
class NotificationBase(BaseModel):
    ...
    status: NotificationStatus = NotificationStatus.PENDING # Enum으로 변경
